...
class VoronoiCanvas(FigureCanvas):
    ...
    def plot_two_points(self):
        ...
        # 繪製法向量線
        self.edges.append((x_values[0], y_values[0], x_values[1], y_values[1]))
        # self.ax.plot(x_values, y_values, 'black')
        self.plot_lines()
        self.draw()
    
    # 如果有三個點
    def plot_three_points(self):
        # 選取最近添加的三個點
        ...
        if a12 == None and a23 == None: # 三點在同一條垂直線上
            ...
            if (y1 > y2 and y1 < y3) or (y1 < y2 and y1 > y3): # y1為中間點
                self.edges.append((0, mid_y12, 600, mid_y12))
                self.edges.append((0, mid_y13, 600, mid_y13))
            elif (y2 > y1 and y2 < y3) or (y2 < y1 and y2 > y3): # y2為中間點
                self.edges.append((0, mid_y12, 600, mid_y12))
                self.edges.append((0, mid_y23, 600, mid_y23))
            ...
            if (x1 > x2 and x1 < x3) or (x1 < x2 and x1 > x3): # x1為中間點
                self.edges.append((mid_x12, 0, mid_x12, 600))
                self.edges.append((mid_x13, 0, mid_x13, 600))
            elif (x2 > x1 and x2 < x3) or (x2 < x1 and x2 > x3): # x2為中間點
                self.edges.append((mid_x12, 0, mid_x12, 600))
                self.edges.append((mid_x23, 0, mid_x23, 600))
            ...
            if (y1 > y2 and y1 < y3) or (y1 < y2 and y1 > y3): # y1為中間點
                ...
                self.edges.append((temp[0][0], temp[0][1], temp[1][0], temp[1][1]))
                ...
                a, b = self.perpendicular_line_params(x2, y2, x3, y3)
                temp = self.border_points(a, b)
                self.edges.append((temp[0][0], temp[0][1], temp[1][0], temp))
        else:
            ...
            if triangle_type == 2: # 銳角
                ...
                self.edges.append((x, y, temp[0], temp[1]))
